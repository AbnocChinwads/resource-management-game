...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const totalWorkers = buildingsRes.rows.reduce((sum, b) => sum + b.workers_assigned, 0);
    const playerRes = await db.query(`SELECT population FROM players WHERE id = $1`, [playerId]);
